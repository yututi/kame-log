...
functions.cloudEvent("capture", async () => {
  ...
  await capture({
    dirname: "shelter",
    deviceId: process.env.METER_AT_SHELTER,
    now,
  });
  await capture({
    dirname: "baskingspot",
    deviceId: process.env.METER_AT_BASKINGSPOT,
    now,
  });
...
export const revalidate = (date) => {
  const body = JSON.stringify({ date: date.format("YYYY-MM-DD") });
  return fetch(process.env.REVALIDATE_URL, {
    method: "POST",
    body,
  }).then((res) => res.json());
};
