...
export default function Menu({
  ...
  return (
    <div style={style} className={`${styles.menu} ${themeStyle[theme]}`}>
      {children}
    </div>
  )
}
...
function Item({ children }: PropsWithChildren) {
  return <div className={`${styles.item} ${commonStyles.highlight}`}>{children}</div>
}
Menu.Item = Item
...
function ButtonItem({ onClick, children }: PropsWithChildren<ButtonProps>) {
  return (
    <div onClick={onClick} className={`${styles.item} ${commonStyles.highlight}`}>
      {children}
    </div>
  )
}
...
function LinkItem({ children, ...props }: PropsWithChildren<LinkProps>) {
  return (
    <Link {...props} className={`${styles.item} ${commonStyles.highlight}`}>
      {children}
    </Link>
  )
}
